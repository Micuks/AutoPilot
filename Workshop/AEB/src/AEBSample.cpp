    ...
    while (true) {
        ...
        if (SimOneAPI::GetCaseRunStatus() == SimOne_Case_Status::SimOne_Case_Status_Running && pObstacle->timestamp > 0 && pGps->timestamp > 0) {
            ...
            if (isObstalceBehind) {
                ...
                double defautlTimeToCollision = 6.4f;
                ...
                if (inAEBState) {
                    ...
                    SimOneAPI::bridgeLogOutput(ELogLevel_Type::ELogLevelDebug, "Accel: %f, calculatedAcceleration: %f, distance: %f", pGps->accelX, accel, std::abs(minDistance));
                        //	pControl->throttle = 0.;
                }
            }	
